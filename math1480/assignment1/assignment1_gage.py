import matplotlib.pyplot as plt            #import the numpy library as np
import numpy as np                         #import the plotting library as plt      
...
# Hint: x = np.array([])
#       y = np.array([])
x = np.array([-1, 0, 2])                       #x-coordinates
y = np.array([9, 4, 11])                       #y-coordinates
...
def quadratic(X):
    q = y[0]*(X-x[1])*(X-x[2])/(x[0]-x[1])/(x[0]-x[2]) + \
...
plt.plot(X, quadratic(X), color='magenta')
plt.plot(X, Q, '--', linewidth=5, color='green')
