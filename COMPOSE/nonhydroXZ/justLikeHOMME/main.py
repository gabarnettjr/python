...
def setGhostNodes(U):
    ...
    thetaBar, pHydro, dpidsBar, dthetaBarDz = fastBackgroundStates(phi/g \
    , U[3,1:-1,:])
...
def odefun(t, U, dUdt):
    ...
    * (Da((pHydro[:-1,:]+pHydro[1:,:])/2.)+Da(U[5,:,:])[1:-1,:]) \
    ...
    - U[1,1:-1,:] * dthetaBarDz[1:-1,:] \
    + HVa(U[2,1:-1,:]) \
    + HVs(U[2,:,:])                                                 #dth/dt
    # - U[1,1:-1,:] * Ds(thetaBar)[1:-1,:] * g/((U[4,1:-1,:]-U[4,0:-2,:])/ds) \
