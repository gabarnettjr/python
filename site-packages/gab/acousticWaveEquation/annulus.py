...
def getHeightCoordinate( rSurf, rSurfPrime, rTop, rBot ) :
...
, rhoB, rhoT, wIinner, wEinner, wIouter, wEouter ) :
    ...
    tmpB = rhoB
    tmpT = rhoT
    for i in np.arange(1,len(wIinner)) :
        tmpB = tmpB - wIinner[i]*U[0,i,:]
        tmpT = tmpT - wIouter[i]*U[0,-i-1,:]
    U[0,0,:]  = tmpB / wIinner[0]
    U[0,-1,:] = tmpT / wIouter[0]
    
    tmpB = 0.
    tmpT = 0.
    for i in np.arange(0,len(wEinner)) :
        tmpB = tmpB + wEinner[i]*U[1,i+1,:]
        tmpT = tmpT + wEouter[i]*U[1,-i-2,:]
    U[1,0,:]  = tmpB
    U[1,-1,:] = tmpT
    ...
    tmpT = 0.
    for i in np.arange(0,len(wEinner)) :
        tmpB = tmpB + wEinner[i]*U[2,i+1,:]
        tmpT = tmpT + wEouter[i]*U[2,-i-2,:]
    U[2,0,:]  = tmpB
    U[2,-1,:] = tmpT
