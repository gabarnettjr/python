...
def getWeights( z=0., x=np.arange(-3.,4.,1.), m=1, phs=5, pol=3 \
...
, phs=5, pol=3, stc=7):
    ...
    for i in range(ell) :
        w[i,:] = getWeights(z=z[i], x=x[jj[i,:]], m=m, phs=phs, pol=pol \
        , P=P, A=A, b=b)
...
def speedtest_getWeights():
    
    def myTimer(N, Z, P, A, b):
        start = time.time()
        for i in range(N):
            w = getWeights(z = Z[i], P = P, A = A, b = b)
