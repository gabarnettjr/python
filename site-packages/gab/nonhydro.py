...
, wx, j0, j1, dx, FD, FDo2 ) :
    ...
    tmp = g/Cp/thetaBarBot**2 * dsdzBot * (U[2,0,jj]+U[2,1,jj])/2.
    U[3,0,jj] = U[3,1,jj] - ds/normGradS**2. * ( tmp - dpidx*dsdxBot )
    
    #get pi on top ghost nodes:
    tmp = g/Cp/thetaBarTop**2 * dsdzBot * (U[2,nLev,jj]+U[2,nLev+1,jj])/2.
    U[3,nLev+1,jj] = U[3,nLev,jj] + ds/dsdzBot**2. * ( tmp - 0. ) 
...
, dsdx, dsdz ) :
    ...
    -Cp*U[2,i0:i1,j0:j1] * ( Dx(U[3,i0:i1,:]) + dsdx*Ds(U[3,:,j0:j1]) )
    
    V[1,:,:] = -U[0,i0:i1,j0:j1]*Dx(U[1,i0:i1,:]) - sDot*Ds(U[1,:,j0:j1]) \
    -Cp*U[2,i0:i1,j0:j1] * ( dsdz*Ds(U[3,:,j0:j1]) )
...
, i0, i1, j0, j1 ) :
    ...
    U[:,i0:i1,j0:j1] = U[:,i0:i1,j0:j1] - dt * implicitPart( U )
...
def leapfrogTimestep( t, U0, U1, dt, nLev, nCol, FD, i0, i1, j0, j1 \
