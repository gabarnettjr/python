...
#Apply a differntial operator (first derivative or HV) vertically:
...
def odefunFD( t, U, setGhostNodes \
, dx, ds, wx, ws, wxhv, wshv, Lx, Ls \
, ii, jj, i0, i1, j0, j1 \
, dsdx, dsdz, FD, FDo2 \
, Cp, Cv, Rd, g ) :
    ...
    if FD == 2 :
        ...
        + (1./2.) * np.abs(u) * Lx( U[:,ii,:], wxhv, jj, dx, FD, FDo2 )
    elif FD == 4 :
        V[ :, i0:i1, j0:j1 ] = V[:,ii,:][:,:,jj] \
        - (1./12.) * np.abs(u) * Lx( U[:,ii,:], wxhv, jj, dx, FD, FDo2 )
    else :
        sys.exit( "\nError: FD should be 2 or 4.\n" )
    V[ :, i0:i1, j0:j1 ] = V[:,ii,:][:,:,jj] \
    + (1./2.) * np.abs(sDot) * Ls( U[:,:,jj], wshv, ii, ds )
