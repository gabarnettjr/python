...
, wx, jj, dx, FD, FDo2 ) :
    ...
    dpdx = LxFD( P[1:3,:], wx, jj, dx, FD, FDo2 )
...
, Cp, Cv, Rd, g, gamma ) :
    ...
    V[ 0, i0:i1, j0:j1 ] = V[0,ii,:][:,jj] + g / U[3,ii,:][:,jj] / dsdz * ( Dx(P[ii,:]) + dpds*dsdx )
    V[ 1, i0:i1, j0:j1 ] = V[1,ii,:][:,jj] - g * ( 1 - dpds/U[3,ii,:][:,jj] )
    # V[ 3, i0:i1, j0:j1 ] = V[3,ii,:][:,jj] - U[3,ii,:][:,jj] * ( Ux[0,:,:]+Us[0,:,:]*dsdx + Us[1,:,:]*dsdz )
    V[ 3, i0:i1, j0:j1 ] = V[3,ii,:][:,jj] - U[3,ii,:][:,jj] * ( Ux[0,:,:] + Ds(sDotAll[:,jj]) )
