...
, dsdz ) :
    ...
    if formulation == "exner" :
        ...
        U[3,:,:] = -g / Rd / (thetaBar+U[2,:,:]) / dsdz(x,z) * Po * (piBar+piPrime0)**(Cv/Rd)
        U[3,:,:] = U[3,:,:] - dpidsBar
    else :
        sys.exit( "\nError: formulation should be 'exner' or 'hydrostaticPressure'.\n" )
    
    return U, thetaBar, piBar, dthetabarDz, dpidsBar
...
def LxFD_2D( U, w, j0, j1, dx, FD, FDo2 ) :
...
def LxFD_3D( U, w, j0, j1, dx, FD, FDo2 ) :
    V = w[0] * U[:,:,(j0-FDo2):(j1-FDo2)]
    for i in range(1,FD+1) :
        V = V + w[i] * U[:,:,(j0-FDo2+i):(j1-FDo2+i)]
    return V
...
, wx, j0, j1, dx, FD, FDo2 ) :
    ...
    dpidx = LxFD_2D( U[3,1:3,:], wx, j0, j1, dx, FD, FDo2 )
    dpidx = 3./2.*dpidx[0,:] - 1./2.*dpidx[1,:]
    th = ( U[2,0,jj] + U[2,1,jj] ) / 2.
    # gOverCpOverTh = g / Cp / th
    thBar = ( thetaBar[0,jj] + thetaBar[1,jj] ) / 2.
    gOverCpOverTh = g/Cp/thBar * ( 2. - th/thBar )
    U[3,0,jj] = U[3,1,jj] + ds/normGradS**2. * ( gOverCpOverTh*dsdzBottom + dpidx*dsdxBottom )
    
    #get pi on top ghost nodes:
    th = ( U[2,nLev,jj] + U[2,nLev+1,jj] ) / 2.
    thBar = ( thetaBar[nLev,jj] + thetaBar[nLev+1,jj] ) / 2.
    U[3,nLev+1,jj] = U[3,nLev,jj] - ds/dsdzBottom*g/Cp/thBar * ( 2. - th/thBar )
...
def implicitPart( U, Lx, Ls, HVx, HVs, nLev, nCol, FD \
, i0, i1, j0, j1, Cp, g, thetaBar, piBar, dthetabarDz, dsdx, dsdz ) :
    
    V = np.zeros(( 4, nLev+2, nCol+FD ))
    
    V[0,i0:i1,j0:j1] = -Cp*thetaBar * ( Lx(U[4,i0:i1,:]) + dsdx*Ls(U[4,:,j0:j1]) ) \
    + HVx(U[0,i0:i1,:]) + HVs(U[0,:,j0:j1])
    
    V[1,i0:i1,j0:j1] = -Cp*thetaBar * dsdz*Ls(U[4,i0:i1,:]) \
    +g/thetaBar * U[2,i0:i1,j0:j1] \
    + HVx(U[1,i0:i1,:]) + HVs(U[1,:,j0:j1])
    
    V[2,i0:i1,j0:j1] = -dthetabarDz * U[1,i0:i1,j0:j1] \
    + HVx(U[2,i0:i1,:]) + HVs(U[2,:,j0:j1])
    
    V[3,i0:i1,j0:j1] = g/Cp/thetaBar * U[1,i0:i1,j0:j1] \
    -Rd/Cv*piBar * ( Lx(U[0,i0:i1,:])+dsdx*Ls(U[0,:,j0:j1]) + dsdz*Ls(U[1,:,j0:j1]) ) \
...
#(I-dt*implicitPart(U))*U

def L( U, dt, setGhostNodes, implicitPart, nLev, nCol ) :
    
    U = np.transpose( np.reshape( U, (4,nCol+FD,nLev+2) ), (0,2,1) )
    
    U = setGhostNodes( U )
    
    U = U - dt * implicitPart( U )
    
    U = np.transpose( U, (0,2,1) ) . flatten()
...
, L, implicitPart, explicitPart, gmresTol ) :
    ...
    V = ( U0 + dt*implicitPart(U0) ) + 2*dt*explicitPart(U1)
    
    U2, flag = gmres( L, V, U1, gmresTol )
...
def odefun1( t, U \
, setGhostNodes, Dx, Ds, HVx, HVs, Dhv, Dz \
, ii, jj, i0, i1, j0, j1 \
, dsdx, dsdz, rbfDerivatives \
, Cp, Cv, Rd, g, gamma \
, V ) :
    ...
    if rbfDerivatives == 0 :
        ...
        u    = U[0,ii,:][:,jj]
        sDot = u * dsdx + U[1,ii,:][:,jj] * dsdz
        gammaAbsU   = gamma * np.abs(u)
        halfAbsSdot = 1./2. * np.abs(sDot)
        CpTh = Cp * U[2,ii,:][:,jj]
        
        V[ 0, i0:i1, j0:j1 ] = -u * Ux[0,:,:] - sDot * Us[0,:,:] \
        - CpTh * ( Ux[3,:,:] + Us[3,:,:]*dsdx ) \
        + gammaAbsU * Uhvx[0,:,:] + halfAbsSdot * Uhvs[0,:,:]
        
        V[ 1, i0:i1, j0:j1 ] = -u * Ux[1,:,:] - sDot * Us[1,:,:] \
        - CpTh * ( Us[3,:,:]*dsdz ) - g \
        + gammaAbsU * Uhvx[1,:,:] + halfAbsSdot * Uhvs[1,:,:]
        
        V[ 2, i0:i1, j0:j1 ] = -u * Ux[2,:,:] - sDot * Us[2,:,:] \
        + gammaAbsU * Uhvx[2,:,:] + halfAbsSdot * Uhvs[2,:,:]
        
        V[ 3, i0:i1, j0:j1 ] = -u * Ux[3,:,:] - sDot * Us[3,:,:] \
        - Rd/Cv*U[3,ii,:][:,jj] * ( Ux[0,:,:]+Us[0,:,:]*dsdx + Us[1,:,:]*dsdz ) \
        + gammaAbsU * Uhvx[3,:,:] + halfAbsSdot * Uhvs[3,:,:]
        ...
        w = np.tile( U[1,ii,:][:,jj], (4,1,1) )
        
        V[ :, i0:i1, j0:j1 ] = - u * Ux - w * Uz \
        + gamma * np.sqrt( u**2 + w**2 ) * Dhv(U)
        
        V[ 0, i0:i1, j0:j1 ] = V[0,ii,:][:,jj] - Cp * U[2,ii,:][:,jj] * Ux[3,:,:]
        V[ 1, i0:i1, j0:j1 ] = V[1,ii,:][:,jj] - Cp * U[2,ii,:][:,jj] * Uz[3,:,:] - g
        V[ 3, i0:i1, j0:j1 ] = V[3,ii,:][:,jj] - Rd/Cv * U[3,ii,:][:,jj] * ( Ux[0,:,:] + Uz[1,:,:] )
...
, xLeft, xRight, zTop, dx, ds ) :
    ...
    if var == 0 :
        ...
        if formulation == "exner" :
            ...
            plt.contourf( x, z, U[3,:,:]-dpidsBar, 20 )
        else :
            sys.exit( "\nError: formulation should be 'exner' or 'hydrostaticPressure'.\n" )
    else :
...
, thetaBar, piBar, dpidsBar ) :
    ...
    if formulation == "exner" :
        ...
        print( "minmaxDpids =", [ np.min(U[3,:,:]-dpidsBar),    np.max(U[3,:,:]-dpidsBar)    ] )
    else :
        sys.exit( "\nError: formulation should be 'exner' or 'hydrostaticPressure'.\n" )
