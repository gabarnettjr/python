...
, dsdx, dsdz, thetaBar, piBar, dthetabarDz ) :
    ...
    # V[0,:,:] = -Cp*thetaBar * ( Dx(U[3,ii,:]) + Ds(U[3,:,:][:,jj])*dsdx )
    # V[0,:,:] = V[0,:,:] + HVx(U[0,ii,:]) + HVs(U[0,:,:][:,jj])
    
    # V[1,:,:] = -Cp*thetaBar * Ds(U[3,:,:][:,jj])*dsdz \
    # +g/thetaBar * U[2,ii,:][:,jj] \
    # + HVx(U[1,ii,:]) + HVs(U[1,:,:][:,jj])
    
    # V[2,:,:] = -U[1,ii,:][:,jj] * dthetabarDz \
    # + HVx(U[2,ii,:]) + HVs(U[2,:,:][:,jj])
    
    # V[3,:,:] = g/Cp/thetaBar * U[1,ii,:][:,jj] \
    # -Rd/Cv*piBar * ( Dx(U[0,ii,:])+Ds(U[0,:,:][:,jj])*dsdx + Ds(U[1,:,:][:,jj])*dsdz ) \
    # + HVx(U[3,ii,:]) + HVs(U[3,:,:][:,jj])
...
, dsdx, dsdz ) :
    ...
    * ( Dx(U[0,i0:i1,:])+Ds(U[0,:,j0:j1])*dsdx + Ds(U[1,:,j0:j1])*dsdz )
    ...
    # V[0,:,:] = -U[0,ii,:][:,jj]*Dx(U[0,ii,:]) - sDot*Ds(U[0,:,:][:,jj]) \
    # -Cp*U[2,ii,:][:,jj] * ( Dx(U[3,ii,:]) + Ds(U[3,:,:][:,jj])*dsdx )
    
    # V[1,:,:] = -U[0,ii,:][:,jj]*Dx(U[1,ii,:]) - sDot*Ds(U[1,:,:][:,jj]) \
    # -Cp*U[2,ii,:][:,jj] * ( Ds(U[3,:,:][:,jj])*dsdz )
    
    # V[2,:,:] = -U[0,ii,:][:,jj]*Dx(U[2,ii,:]) - sDot*Ds(U[2,:,:][:,jj])
    
    # V[3,:,:] = -U[0,ii,:][:,jj]*Dx(U[3,ii,:]) - sDot*Ds(U[3,:,:][:,jj]) \
    # -Rd/Cv*U[3,ii,:][:,jj] \
    # * ( Dx(U[0,ii,:])+Ds(U[0,:,:][:,jj])*dsdx + Ds(U[1,:,:][:,jj])*dsdz )
...
#(I-dt*implicitPart(U))*U on the main nodes, and sets the ghost node values
#to what they should be.

def L( U, dt, setGhostNodes, implicitPart, nLev, nCol, FD \
, i0, i1, j0, j1 ) :
    
    tmp = np.transpose( np.reshape( U.flatten(), (4,nCol+FD,nLev+2) ), (0,2,1) )
    
    U, P = setGhostNodes( tmp )
    
    U = tmp - U
    
    U[:,i0:i1,j0:j1] = tmp[:,i0:i1,j0:j1] - dt * implicitPart( tmp )
...
, L, implicitPart, explicitPart, gmresTol ) :
    ...
    V[:,i0:i1,j0:j1] = ( U0[:,i0:i1,j0:j1] + dt*implicitPart(U0) ) + 2.*dt*explicitPart(U1)
    
    V  = np.transpose( V,  (0,2,1) ) . flatten()
    U1 = np.transpose( U1, (0,2,1) ) . flatten()
    
    U2, flag = gmres( L, V, x0=U1, tol=gmresTol )
...
def printInfo( U, et , t, formulation ) :
