...
def parseInput() :
    ...
    # parser.add_argument("--Rd", help="gas constant" \
    # , type=np.float64, default=287)
    
    parser.add_argument("--tf", help="final simulation time in seconds" \
    , type=np.float64, default=1000.)
    
    parser.add_argument("--exp", help="controls steepness of initial \
    Gaussian bell(s)" \
    ...
    parser.add_argument("--clu", help="type of angular clustering.  \
    Choose 'linear' or 'geometric' if you want clustering." \
    , type=str,        default='geometric')
    
    parser.add_argument("--ptr", help="random node perturbation \
    percentage in the radial direction" \
...
def getSavestring( innerRadius, outerRadius, tf, saveDel, exp, amp, frq \
, mlv, phs, pol, stc, clu, ptr, rks, nlv, dti ) :
    
    saveString                                  \
    = 'ri'   + '{0:1.0f}' . format(innerRadius) \
...
, wIinner, wEinner, wDinner, wHinner, wIouter, wEouter, wDouter, wHouter ) :
    ...
    U[4,1:-1,:] = ( rhoBar[1:-1,:] + U[3,1:-1,:] ) * Rd \
    * ( Tbar[1:-1,:] + U[2,1:-1,:] ) - Pbar[1:-1,:]
    
    #################################
    
    #Enforce dP/dN=-rho*G on inner boundary:
    RHS = wHinner.dot( U[4,1:stcB+1,:] )          #extrap P to bottom bndry
    ...
    # RHS = RHS - rho * Gbot / normgradsBot
    U[4,0,:] = ( RHS - wDinner[1:stcB].dot(U[4,1:stcB,:]) ) / wDinner[0]
    ...
    U[3,0,:] = ( U[4,0,:] + Pbar[0,:] ) / Rd \
    / ( U[2,0,:] + Tbar[0,:] ) - rhoBar[0,:]
    # U[3,0,:] = wEinner.dot( U[3,1:stcB+1,:] )
    
    #################################
    
    #Enforce dP/dN=-rho*G on outer boundary:
    # rho = wHouter.dot( U[3,-2:-(stcB+2):-1,:] )
    # RHS = -rho * Gtop / normgradsTop
    U[4,-1,:] = ( 0. - wDouter[1:stcB].dot( U[4,-2:-(stcB+1):-1,:] ) ) / wDouter[0]
...
, Tbar, rhoBar, dTbarDr, drhoBarDr, Rd, Cv, g ) :
    ...
    #duDt:
    dUdt[0,1:-1,:] = ( -U[0,:,:]*Dx(U[0,:,:]) - U[1,:,:]*Dy(U[0,:,:]) \
    - 1. / rho * Dx(U[4,:,:]) \
    - U[3,:,:] / rho * g * drdxAll ) [1:-1,:] \
    + HV(U[0,:,:])
    
    #dvDt:
    dUdt[1,1:-1,:] = ( -U[0,:,:]*Dx(U[1,:,:]) - U[1,:,:]*Dy(U[1,:,:]) \
    - 1. / rho * Dy(U[4,:,:]) \
    - U[3,:,:] / rho * g * drdyAll ) [1:-1,:] \
    + HV(U[1,:,:])
    ...
    - uDotGradR * drhoBarDr - rho * divU ) [1:-1,:] \
    + HV(U[3,:,:])
...
, Tbar, rhoBar, dTbarDr, drhoBarDr, g ) :
    ...
    dUdt[0,1:-1,:] = ( - 1. / rhoBar * Dx(U[4,:,:]) ) [1:-1,:] \
    + HV(U[0,:,:])
    
    dUdt[1,1:-1,:] = ( - 1. / rhoBar * Dy(U[4,:,:]) ) [1:-1,:] \
    + HV(U[1,:,:])
    
    divU = Dx(U[0,:,:]) + Dy(U[1,:,:])
    
    dUdt[3,1:-1,:] = ( - rhoBar * divU ) [1:-1,:] \
    + HV(U[3,:,:])
...
# def odefunCartesian( t, U, dUdt   \
# , setGhostNodes, Dx, Dy, HV \
# , gx, gy, wavesOnly, Tbar, rhoBar, Pbar ) :
#     
#     U = setGhostNodes(U)
#     
#     dUdt[0,1:-1,:] = ( -1./(rhoBar+U[3,:,:])*Dx(Pbar+U[4,:,:]) ) [1:-1,:] \
#     + HV( U[0,:,:] )
#     
#     dUdt[1,1:-1,:] = ( -1./(rhoBar+U[3,:,:])*Dy(Pbar+U[4,:,:]) ) [1:-1,:] \
#     + HV( U[1,:,:] )
#     
#     divU = Dx(U[0,:,:]) + Dy(U[1,:,:])
#     
#     dUdt[3,1:-1,:] = ( -(rhoBar+U[3,:,:]) * divU ) [1:-1,:] \
#     + HV( U[3,:,:] - rhoBar )
#     
#     if not wavesOnly :
#     
#         dUdt[0,1:-1,:] = dUdt[0,1:-1,:] \
#         - ( gx + U[0,:,:]*Dx(U[0,:,:]) + U[1,:,:]*Dy(U[0,:,:]) ) [1:-1,:]
#         
#         dUdt[1,1:-1,:] = dUdt[1,1:-1,:] \
#         - ( gy + U[0,:,:]*Dx(U[1,:,:]) + U[1,:,:]*Dy(U[1,:,:]) ) [1:-1,:]
#         
#         dUdt[2,1:-1,:] = 
#         -(Tbar+U[2,1:-1,:])*divU \
#         + HV( U[2,:,:] - Tbar )
#         
#         dUdt[3,1:-1,:] = dUdt[3,1:-1,:] \
#         + ( - U[0,:,:]*Dx(U[3,:,:]) - U[1,:,:]*Dy(U[3,:,:]) \
#         - (rhoBar+U[3,:,:])*divU ) [1:-1,:]
#     
#     return dUdt
...
, Tbar, rhoBar, thetaBar, Pbar, piBar ) :
    ...
    if whatToPlot == "P" :
        # tmp = Pbar
        tmp = U[4,:,:]
        # cvec = np.arange( 24.75e-5, 36.25e-5, .5e-5 )
        cvec = np.arange( 80000, 101000, 1000 )
    elif whatToPlot == "pi" :
        # tmp = piBar
        tmp = ( (U[4,:,:]+Pbar) / Po ) ** (Rd/Cp) - piBar
        ...
        tmp = Dx(U[0,:,:]) + Dy(U[1,:,:])
        cvec = np.linspace( -Rd, Rd, 20 )
    elif whatToPlot == "rho" :
        # tmp = rhoBar
        tmp = U[3,:,:]
        cvec = np.arange( 1.-.155, 1.+.165, .01 )
    elif whatToPlot == "T" :
        # tmp = Tbar
        tmp = U[2,:,:]
        cvec = np.arange( 200., 325., 5. )
    elif whatToPlot == "vMag" :
        tmp = np.sqrt( U[0,:,:]**2. + U[1,:,:]**2. )
        cvec = np.arange( 0., .0021, .0001 )
    elif whatToPlot == "theta" :
        # tmp = thetaBar
        tmp = (U[2,:,:]+Tbar) / ((U[4,:,:]+Pbar)/Po)**(Rd/Cp) - thetaBar
    ...
    if noInterp :
        ...
        th = np.hstack(( th, th[0] ))
        tmp = np.vstack(( tmp.T, tmp[:,0] )).T
        ind1 = ( th > ang1+2*np.pi-alp )
        ind2 =  ( th > ang1 ) & ( th < ang1+alp )
        ...
        tmp = np.vstack(( tmp.T, tmp[:,0] )).T
        # ind1 = ( th0 > ang1+2*np.pi-alp )
        # ind2 =  ( th0 > ang1 ) & ( th0 < ang1+alp )
