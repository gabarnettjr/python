...
def parseInput() :
    ...
    parser.add_argument("--plotVerticalCoordinate", help="use this if you \
    want to just plot the height coordinate transformation functions and \
    exit" \
    , action="store_true")
...
, outerRadius, Po, Rd, Cp ) :
    ...
    # print(np.min(exnerPressure(rSurf(th))))
    # print(np.max(exnerPressure(rSurf(th))))
    # tmp = exnerPressure((rr[0,:]+rr[1,:])/2)
    # print(np.min(tmp))
    # print(np.max(tmp))

    # print(exnerPressure(outerRadius))
    # tmp = exnerPressure((rr[-1,:]+rr[-2,:])/2)
    # print(np.min(tmp))
    # print(np.max(tmp))

    # outerRadius = ( rr[-1,0] + rr[-2,0] ) / 2
...
, innerRadius, outerRadius, rB, rT, g ) :
    ...
    * ( Tbar[1:-1,:] + U[2,1:-1,:] ) - Pbar[1:-1,:]
    ...
    RHS = RHS - rho * g * (cosTh*NxBot[0,:]+sinTh*NyBot[0,:])
    RHS = RHS / denominator
    ...
    RHS = RHS - rho * g * (cosTh*NxTop[0,:]+sinTh*NyTop[0,:])
    RHS = RHS / denominator
...
Choose P, u, v, curl, div, rho, T, or vMag.")
    ...
    if testCase == "igw" :
        plt.axis([-150000,150000,innerRadius-2000,outerRadius+250])
        plt.colorbar(orientation='horizontal')
    elif testCase == "bubble" :
        plt.axis('image')
        plt.axis([-5000,5000,innerRadius-amp-250,outerRadius+250])
        plt.colorbar(orientation='vertical')
    elif testCase == "densityCurrent" :
        plt.axis('image')
        plt.axis([-25000,25000,innerRadius-250,outerRadius+250])
        plt.colorbar(orientation='horizontal')
    else :
        raise ValueError("haven't done other cases yet.")
