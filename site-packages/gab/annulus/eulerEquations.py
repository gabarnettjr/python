...
def getSavestring( Rd, innerRadius, outerRadius, tf, saveDel, exp, amp, frq \
...
, Tbar, rhoBar, dTbarDr, drhoBarDr, g ) :
    ...
    - 1./(rhoBar+U[3,:,:])*Dx(U[4,:,:]) + g*(1.-drdxAll) ) [1:-1,:]
    + HV(U[0,:,:])
    
    dUdt[1,1:-1,:] = ( -U[0,:,:]*Dx(U[1,:,:]) - U[1,:,:]*Dy(U[1,:,:]) \
    - 1./(rhoBar+U[3,:,:])*Dy(U[4,:,:]) + g*(1.-drdyAll) ) [1:-1,:]
    + HV(U[1,:,:])
    ...
    - uDotGradR*dTbarDr - (Tbar+U[2,:,:])*divU ) [1:-1,:]
    + HV(U[2,:,:])
...
, Tbar, rhoBar, thetaBar, Pbar, piBar ) :
    ...
    if whatToPlot == "P" :
        ...
        cvec = np.linspace(0., 1., 20 )
    elif whatToPlot == "u" :
        tmp = U[0,:,:]
        cvec = np.linspace( -Rd/10., Rd/10., 20 )
    elif whatToPlot == "v" :
        tmp = U[1,:,:]
        cvec = np.arange( -.00155, .00165, .0001 )
