...
def parseInput() :
    ...
    , type=np.float64, default=.000001)
...
, wIinner, wEinner, wDinner, wHinner, wIouter, wEouter, wDouter, wHouter ) :
    ...
    U[4,-1,:] = ( RHS - wDouter[1:stcB].dot( U[4,-2:-(stcB+1):-1,:] ) ) / wDouter[0]
...
, cosTh, sinTh, cosThOverR, sinThOverR, mtx, mty ) :
    ...
    dUdt[0,1:-1,:] = -Po *                                         \
    ( mtx * Ds(U[1,:,:]) - sinThOverR * Dlam(U[1,:,:])             \
    + mty * Ds(U[2,:,:]) + cosThOverR * Dlam(U[2,:,:]) ) [1:-1,:]  \
    + HV(U[0,:,:])
...
, gx, gy, wavesOnly, Tbar, rhoBar ) :
    ...
    dUdt[0,1:-1,:] = ( -1./U[3,:,:]*Dx(U[4,:,:]) ) [1:-1,:] \
    + HV( U[0,:,:] )
    
    dUdt[1,1:-1,:] = ( -1./U[3,:,:]*Dy(U[4,:,:]) ) [1:-1,:] \
    + HV( U[1,:,:] )
    
    divU = ( Dx(U[0,:,:]) + Dy(U[1,:,:]) ) [1:-1,:]
    ...
    if not wavesOnly :
        ...
        dUdt[2,1:-1,:] = -U[2,1:-1,:] * divU \
        + HV( U[2,:,:] - Tbar )
    
    dUdt[3,1:-1,:] = -U[3,1:-1,:] * divU \
    + HV( U[3,:,:] - rhoBar )
