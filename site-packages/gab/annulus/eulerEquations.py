...
def parseInput() :
    ...
    parser.add_argument("--pct", help="controls clustering of nodes in \
...
, wIouter, wEouter, wDouter, wHouter ) :
    ...
    * ( Tbar[1:-1,:] + U[2,1:-1,:] ) - Pbar[1:-1,:]
    ...
    rho = wHinner.dot( U[3,1:stcB+1,:] )        #extrap who to bottom bndry
    RHS = RHS + rho * bottomFactor                  #full RHS of Neumann BC
...
, Tbar, rhoBar, dTbarDr, drhoBarDr, Rd, Cv, g ) :
    ...
    - rhoInv * Dx(U[4,:,:]) - buoyancy * drdxAll ) [1:-1,:] \
    + HV(U[0,:,:])                                                   #du/dt
    
    dUdt[1,1:-1,:] = ( -U[0,:,:]*Dx(U[1,:,:]) - U[1,:,:]*dvdy \
    - rhoInv * Dy(U[4,:,:]) - buoyancy * drdyAll ) [1:-1,:] \
    + HV(U[1,:,:])                                                   #dv/dt
