...
, testCase, nlv, nth, g, Cp, Rd, Po ) :
    ...
    if testCase == "bubble" :
        ...
        raise ValueError('Only bubble test case supported right now.')
...
def verticalRemap( U, z, Z ) :          #used only in vertically Lagrangian
...
, innerRadius, outerRadius, rB, rT, g ) :
    ...
    #Enforce phi = g*(outerRadius-innerRadius) on bottom boundary:
    RHS = g * ( outerRadius - innerRadius )
    U[4,-1,:] = ( RHS - wIouter[1:stcB].dot(U[4,-2:-stcB-1:-1,:]) ) / wIouter[0]
    
    #Get background states on possibly changed geopotential levels:
    Pbar, rhoBar, Tbar, drhoBarDr, dTbarDr = fastBackgroundStates(U[4,:,:])
...
, Tbar, rhoBar, dTbarDr, drhoBarDr, Rd, Cv, g ) :
    ...
    - rhoInv * Dx(U[5,:,:]) - buoyancy * drdxAll ) [1:-1,:] \
    + HV(U[0,:,:])                                                   #du/dt
    
    dUdt[1,1:-1,:] = ( -U[0,:,:]*Dx(U[1,:,:]) - U[1,:,:]*dvdy \
    - rhoInv * Dy(U[5,:,:]) - buoyancy * drdyAll ) [1:-1,:] \
    + HV(U[1,:,:])                                                   #dv/dt
...
, Tbar, rhoBar, thetaBar, Pbar, piBar ) :
    ...
    alp = np.pi/3e3                                   #nearly square region
    # alp = np.pi/4e2                                       #very wide region
