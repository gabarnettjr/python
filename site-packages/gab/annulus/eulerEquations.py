...
def parseInput() :
    ...
    parser.add_argument("--useGravity", help="use this if you want to have gravity" \
    , action="store_true")

    parser.add_argument("--wavesOnly", help="use this if you want acoustic waves only" \
    , action="store_true")
    ...
    parser.add_argument("--whatToPlot", help="choose between P, rho, T, u, v, curl, div, and vMag" \
    , default="P")
    ...
    parser.add_argument("--clu", help="type of angular clustering to use.  \
    Choose 'linear' or 'quadratic' if you want clustering." \
    , type=str,        default='linear')
    
    parser.add_argument("--ptr", help="random node perturbation percentage \
    ...
    + " finite differences (FD8) in the angular direction" \
    , action="store_true")
...
, dynamicColorbar, noInterp ) :
    ...
    if noInterp :
        ...
        ell = np.shape(tmp)[1]
        start = np.int(np.round( ell/2+ell/200 ))
        stop = np.int(np.round( ell/2+ell/100 ))
        plt.contourf( xx0[:,start:stop], yy0[:,start:stop] \
        , tmp[:,start:stop], cvec )
