...
, testCase, nlv, nth, g, Cp, Rd, Po ) :
    ...
    if ( testCase == "bubble" ) | ( testCase == "densityCurrent" ) :
        ...
        N = .01
        theta0 = 300.
        thetaBar = theta0 * np.exp( N**2/g * (phi/g) )
        piBar = 1. + g**2/Cp/theta0/N**2 * ( np.exp(-N**2/g*(phi/g)) - 1. )
        dthetaBarDr = theta0 * N**2/g * np.exp( N**2/g * (phi/g) )
    else :
        raise ValueError("Only 'bubble' and 'densityCurrent' and \
        'mountainWaves' are currently supported.")
