...
, Po, rhoInv ) :
    ...
    dUdt[0,1:-1,:] = -Po * ( Dx(U[1,:,:]) + Dy(U[2,:,:]) ) [1:-1,:] \
    + HV( U[0,:,:] )
    
    dUdt[1,1:-1,:] = -rhoInv * Dx(U[0,:,:])[1:-1,:] \
    + HV( U[1,:,:] )
    
    dUdt[2,1:-1,:] = -rhoInv * Dy(U[0,:,:])[1:-1,:] \
    + HV( U[2,:,:] )
...
, dynamicColorbar, noInterp, figDir ) :
    ...
    if whatToPlot == "P" :
        ...
        Choose P, u, v, curl, div, rho, or vMag.")
    ...
    fig.savefig( figDir + os.sep + '{0:04d}'.format(np.int(np.round(t)+1e-12)) \
