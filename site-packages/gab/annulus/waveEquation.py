...
def parseInput() :
    ...
    parser.add_argument("--innerRadius", help="inner radius of annulus" \
    , type=np.float64, default=1.)
    parser.add_argument("--outerRadius", help="outer radius of annulus" \
    , type=np.float64, default=2.)
    parser.add_argument("--tf",          help="final simulation time in seconds" \
    , type=np.float64, default=200.)
    parser.add_argument("--saveDel",     help="time between saves" \
    , type=np.int64,   default=10)
    parser.add_argument("--exp",         help="controls steepness of initial Gaussian bells" \
    ...
    parser.add_argument("--ptb", help="random radial perturbation percentage" \
    , type=np.int64,   default=0)
    parser.add_argument("--rks", help="number (3 or 4) of Runge-Kutta stages" \
    , type=np.int64,   default=3)
    parser.add_argument("--nlv",  help="total number of radial s levels" \
    , type=np.int64,   default=50)
    parser.add_argument("--dti", help="inverse of delta t (integer)" \
    , type=np.int64,   default=4)
    parser.add_argument("--uhv", help="1:use radial HV, 0:do not use radial HV" \
    , type=np.int64,   default=1)
...
, cosTh, sinTh, cosThOverR, sinThOverR ) :
    ...
    # + sinTh*(v_s*dsdr) + cosThOverR * ( Dlam(U[2,:,:]) + v_s*dsdth ) ) \
    # + HV( U[0,:,:] )
