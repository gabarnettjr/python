...
def getApprox( stencil, A, f ) :
    ...
    f = np.transpose( np.hstack(( f, np.zeros((A.numPoly)) )) )
...
def testThisLibrary() :
    ...
    ptsE = 0.1 + 0.8 * np.random.rand(nE, dim)
    # x = np.linspace( .1, .9, int(np.sqrt(nE)) )
    # y = x.copy()
    ...
    for i in range( len(ptsE) ) :
        ...
        approx[i] = getApprox( stencil, A, F[stencil.idx] )
