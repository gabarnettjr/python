...
# https://www.overleaf.com/read/wwsnbcxqnnsp
...
, g, Cp, normGradS, dsdxBot, dsdzBot ) :
    ...
    dpida = Wa.dot(wIbot.dot(U[3,0:stc,:]).T).T
    # dpida = U[3,0:stc,:] @ Wa
    # dpida = wIbot.dot( dpida )
    tmp = g/Cp/thetaBarBot**2 * dsdzBot * wIbot.dot( U[2,0:stc,:] )
    tmp = 1./normGradS**2. * ( tmp - dpida*dsdxBot )
    U[3,0,:] = ( tmp - wDbot[1:stc].dot(U[3,1:stc,:]) ) / wDbot[0]
    
    #get pi on top ghost nodes:
    tmp = g/Cp/thetaBarTop**2 * dsdzBot * wItop.dot( U[2,-1:-(stc+1):-1,:] )
    tmp = 1./dsdzBot**2. * ( tmp - dpida*0. ) 
    U[3,-1,:] = ( tmp - wDtop[1:stc].dot( U[3,-2:-(stc+1):-1,:] ) ) / wDtop[0] 
...
, dsdx, dsdz, thetaBar, piBar, dthetaBarDz ) :
    ...
    V[0,:,:] = -Cp*thetaBar * ( Da(U[3,:,:]) + dpids*dsdx ) \
    + HV( U[0,:,:] )
    
    V[1,:,:] = -Cp*thetaBar * dpids*dsdz \
    +g/thetaBar * U[2,1:-1,:] \
    + HV( U[1,:,:] )
    ...
    V[3,:,:] = g/Cp/thetaBar * U[1,1:-1,:] \
    -Rd/Cv*piBar * ( Da(U[0,:,:])+Ds(U[0,:,:])*dsdx + Ds(U[1,:,:])*dsdz ) \
    + HV( U[3,:,:] )
...
, dsdx, dsdz ) :
    ...
    -Cp*U[2,1:-1,:] * ( Da(U[3,:,:]) + dpids*dsdx )
    
    V[1,:,:] = -u*Da(U[1,:,:]) - sDot*dwds \
    -Cp*U[2,1:-1,:] * ( dpids*dsdz )
    
    V[2,:,:] = -u*Da(U[2,:,:]) - sDot*Ds(U[2,:,:])
    
    V[3,:,:] = -u*Da(U[3,:,:]) - sDot*dpids \
    -Rd/Cv*U[3,1:-1,:] * ( duda+duds*dsdx + dwds*dsdz )
