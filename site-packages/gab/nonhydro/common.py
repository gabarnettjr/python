...
def constants(testCase):
    ...
    Po = 10.**5.                     #reference surface pressure in Pascals

    if (testCase == "tortureTest"):
        th0 = 288.
    else:
...
def domainParameters(testCase, hydrostatic, refinementLevel, g, Cp, th0):
    """
    ...
    """
    if (testCase == "risingBubble") or (testCase == "steadyState"):
    ...
    elif testCase == "tortureTest":             #DCMIP 2012 Test Case 2-0-0
        ...
        bottom  = 0.
        top     = 12000.
        nLev    = 15 * 2**refinementLevel           #15 levels or 30 levels
        tf      = 6. *24.*60.*60.                                   #6 days
        saveDel = 6. * 60.*60.                                     #6 hours
        if hydrostatic:
            dt = 5.*60.                                          #5 minutes
        else:
            dt = 1. / 2**refinementLevel            #1 second or 1/2 second
        ...
        def zSurf(x):
            ...
            ind = np.abs(x) <= Rm
            h[ind] = h0/2. * (1. + np.cos(np.pi*x[ind]/Rm))
            return np.cos(np.pi*x/Gm)**2. * h
        ...
        #     h = np.zeros(np.shape(x))
        #     ind = np.abs(x) <= a
        #     h[ind] = h0 * np.cos(np.pi*x[ind]/2./a)**2.
        #     return np.cos(np.pi*x/lam)**2. * h
        #     # return np.zeros(np.shape(x))

        def thetaPtb(x, z):
            return np.zeros(np.shape(x))

    elif testCase == "scharMountainWaves":         #Schar 2002 test case b.
        ...
        dx      = 500.
        nLev    = 65
        dt      = 1. / 4.                       #Schar claims to use dt = 8
        tf      = 7200.
        saveDel = 300.
        ...
        def zSurf(x):
            ...
            a = 5000.
            lam = 4000.
            return h0 * np.exp(-(x/a)**2.) * (np.cos(np.pi*x/lam))**2.
        ...
        + "tortureTest, or scharMountainWaves.")
...
def hydrostaticProfiles(testCase, th0, Po, g, Cp, Rd, N):
    """
    ...
    or (testCase == "steadyState"):
    ...
    elif (testCase == "inertiaGravityWaves") or (testCase == "tortureTest"):
        ...
        def exnerPressure(z):
            return np.exp(-g * z / Cp / Teq)

        def inverseExnerPressure(pi):
            return -Cp * Teq / g * np.log(pi)

        def potentialTemperature(z):
            return Teq / exnerPressure(z)
...
def verticalRemap(U, z, Z, V):          #used only in vertically Lagrangian
    ...
    V is the output array (same size as U)
    """
    nLev = np.shape(z)[0] - 2
...
def tangentsAndNormals(zSurfPrime, stc):
    ...
    """
    NxBot = np.tile(NxBot, (stc-1,1)) #x-component of unit normal on bottom
    NzBot = np.tile(NzBot, (stc-1,1)) #z-component of unit normal on bottom
...
def printMinAndMax(t, et, U, rhoBar, phiBar):
