...
, dsdz ) :
    ...
    if testCase == "bubble" :
        ...
        ind1 = rTilde1 <= 1
        ind2 = rTilde2 <= 1
        Tprime0[ind1] = -15. * np.exp( -(2.*rTilde1[ind1])**2. )
        Tprime0[ind2] = Tprime0[ind2] - 15. * np.exp( -(2.*rTilde2[ind2])**2. )
    ...
    if formulation == "theta_pi" :
        ...
        U[3,:,:] = -g / Rd / (thetaBar+thetaPrime0) / dsdz(x,z) * Po * (piBar+piPrime0)**(Cv/Rd)
        U[4,:,:] = g * z
    else :
