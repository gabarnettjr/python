...
, g, Cp, Cv, Rd, Po, backgroundStatesTmp ) :
    ...
    if testCase == "igw" :
        ...
        backgroundStatesTmp[1,:,:] = -g / Rd / backgroundStatesTmp[0,:,:] \
        / dsdz(x,z) * Po * piBar**(Cv/Rd)
        backgroundStatesTmp[2,:,:] = Po * piBar ** (Cp/Rd)
        backgroundStatesTmp[3,:,:] = (N**2/g) * backgroundStatesTmp[0,:,:]
    elif ( testCase == "bubble" ) | ( testCase == "doubleDensityCurrent" ) :
        ...
        sys.exit("\nError:  Other cases aren't included yet.\n")
...
, Po, g, Rd, Cv, Cp, zBot, zTop, x, z, VL ) :
    ...
    P[1:-1,:] = ( -Rd/Po**(Rd/Cp) * (U[2,1:-1,:]+thetaBar[1:-1,:]) \
    * (U[3,1:-1,:]+dpidsBar[1:-1,:]) \
    / (Ws[1:-1,:].dot(U[4,:,:])) ) ** (Cp/Cv) - Pbar[1:-1,:]
    
    #get pressure on bottom ghost nodes using phi-based BC enforcement:
    dpids = wHbot.dot( U[3,1:stc+1,:] )
    dPda = Wa.dot(wHbot.dot(P[1:stc+1,:]).T).T
    dphida = Wa.dot(wIbot.dot(U[4,0:stc,:]).T).T
    dphids = wDbot.dot(U[4,0:stc,:])
    tmp = 1. / ( dphida**2. + g**2. ) * ( g**2.*dpids + dphida*dphids*dPda )
    P[0,:] = ( tmp - wDbot[1:stc].dot(P[1:stc,:]) ) / wDbot[0]
    
    #get pressure on top ghost nodes using phi-based BC enforcement:
    dpids = wHtop.dot( U[3,-2:-stc-2:-1,:] )
    dPda = Wa.dot(wHtop.dot(P[-2:-stc-2:-1,:]).T).T
    dphida = Wa.dot(wItop.dot(U[4,-1:-stc-1:-1,:]).T).T
    dphids = wDtop.dot(U[4,-1:-stc-1:-1,:])
    tmp = 1. / ( dphida**2. + g**2. ) * ( g**2.*dpids + dphida*dphids*dPda )
...
, nLev, nCol, g, wIbot, wItop, stc, z, VL ) :
    ...
    + dphids / (dpidsBar[1:-1,:]+U[3,1:-1,:]) * (Da(Pbar)+Da(P)) \
    - (Ds(Pbar)+Ds(P)) / (dpidsBar[1:-1,:]+U[3,1:-1,:]) * Da(U[4,:,:]) \
    + HV( U[0,:,:] )
    ...
    -u*Da(thetaBar) - sDot[1:-1,:]*Ds(thetaBar) \
    + HV( U[2,:,:] )
