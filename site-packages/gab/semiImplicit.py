...
def getInitialConditions( testCase, nLev, nCol, x, z , Cp, Cv, Rd, g ) :
    ...
    U0[N:2*N]   = np.zeros(np.shape(x))
    U0[2*N:3*N] = thetaPrime0
    U0[3*N:4*N] = piPrime0
    
    return U0, thetaBar, piBar, dthetabarDz, dpibarDz
...
def getBoundaryConditionMatrices( thetaBar, nLev, nCol, g, Cp ) :
    ...
    thetaBarBot = 3./2.*thetaBar[0,:] - 1./2.*thetaBar[1,:]
    
    thetaBarTop = 3./2.*thetaBar[nLev-1,:] - 1./2.*thetaBar[nLev-2,:]
    
    cBot = g / Cp  / thetaBarBot**2.
    cTop = g / Cp  / thetaBarTop**2.
